...
func New(ctx context.Context, cfg config.Config, logger *slog.Logger) (*Application, error) {
	...
	egressManager.SetDisableCooldown(cfg.Routing.DisableCooldown)
	...
	selector.SetDisableCooldown(cfg.Routing.DisableCooldown)
	...
	settingsService := settingsapp.NewService(cfg, settingsUpdatedAt, settingsRevision, runtimeSettingsRepo, notifySettings, func(next config.Config) {
		...
		selector.SetDisableCooldown(next.Routing.DisableCooldown)
		egressManager.SetDisableCooldown(next.Routing.DisableCooldown)
